...
function renderTareas() {
  ...
  // Ordenar: rojas > estancadas > amarillas > verdes
  ...
  return `
    <div class="task-card ${t.traffic_level}" onclick="openTaskDetail('${t.id}')">
      <div class="task-card-top">
        <div class="task-card-title">${isCompleted ? '<span style="text-decoration:line-through;opacity:0.6">' : ''}${esc(t.title)}${isCompleted ? '</span>' : ''}</div>
      ...
      <div class="task-card-meta">
        ...
        ${t.is_stagnant ? '<span class="stagnant-badge">⚠ Estancada</span>' : ''}
      </div>
      ${!isCompleted ? `
      <div class="task-card-actions" onclick="event.stopPropagation()">
        <button class="btn btn-success btn-sm" onclick="completeTask('${t.id}')">✓ Completar</button>
        <button class="btn btn-ghost btn-sm" onclick="openTaskDetail('${t.id}')">Ver</button>
      </div>` : ''}
    </div>`;
}
